...
def generate_care_timeline(vegetable_name: str, material_names: List[str]) -> dict:
    ...
    chat_prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", "Here is the user's plant information:\n"
                  "- Vegetable: {vegetable}\n"
...
def _parse_period_to_duration(period_string: str) -> relativedelta | None:
    ...
    """
    if not period_string or "not" in period_string.lower() or "as needed" in period_string.lower():
        return None
    ...
    match = re.search(
        r'(\d+)\s+(day|days|week|weeks|month|months)', period_string)
    # ---------------------
    ...
    match_word = re.search(
        r'(one|two|three|four|five|six|seven|eight|nine|ten)\s+(day|days|week|weeks|month|months)', period_string)
    if match_word:
