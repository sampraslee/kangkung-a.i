...
@router.post("/vegetables/", response_model=schemas.Vegetable)
def create_vegetable(vegetable: schemas.VegetableCreate, db: Session = Depends(deps.get_db)):
    return crud.crud_vegetable.create_vegetable(db=db, vegetable=vegetable)


@router.get("/vegetables/", response_model=List[schemas.Vegetable])
def read_vegetables(skip: int = 0, limit: int = 100, db: Session = Depends(deps.get_db)):
...
def create_user_progress(progress: schemas.ProgressCreate, db: Session = Depends(deps.get_db)):
...
def update_user_progress(
    ...
    db_progress = crud.crud_progress.get_progress_by_id(
        db, progress_id=progress_id)
    if not db_progress:
        raise HTTPException(status_code=404, detail="Progress entry not found")
    return crud.crud_progress.update_progress(db=db, db_obj=db_progress, obj_in=progress_update)
