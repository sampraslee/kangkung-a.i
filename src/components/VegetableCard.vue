<template>
  <v-card class="pa-6 ga-4 border-sm rounded-lg d-flex flex-column">
    ...
    <v-container class="vegetableInfo pa-0 ga-3 d-flex flex-column">
      ...
      <v-chip prepend-icon="mdi-clock-time-four-outline" size="large" class="rounded-lg bg-accent">
        Estimated time to harvest: {{ estimatedHarvestTime }}
      </v-chip>
      <v-chip prepend-icon="mdi-water-outline" size="large" class="rounded-lg bg-accent">
        Watering frequency: {{ wateringFrequency }}
      </v-chip>
      <v-chip prepend-icon="mdi-weather-sunny" size="large" class="rounded-lg bg-accent">
        Amount of sunlight: {{ amountOfSunlight }}
      </v-chip>
    </v-container>

    <RouterLink :to="`/vegetablePlantingInfo/`" 
      @click="handleHowToGrowClick"
    >
